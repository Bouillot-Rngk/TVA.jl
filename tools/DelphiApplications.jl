...
const TopoMapsDir = homedir()*"\\Delphi5\\Projects\\@Julia\\TopoMaps"
const ICoNmaxDataSize=100000000000000 # 3573518 tested: OK
const ICoNdir=homedir()*"\\Delphi5\\Projects\\@julia\\WavesEditor"
const CSTPdir=homedir()*"\\Delphi5\\Projects\\@julia\\CSTP"
...
# `mask`: if it is not (), this is a 2-tuple with the following elements:
...
# to its own maximum. If it equal to `:gloabl`(default) all maps are scaled
