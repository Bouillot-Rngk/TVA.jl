...
## normalized regularized Tyler's M-Estimator (Zhang and Wiesel, 2016)  with filtering of Global Field Power
# `X` (the data) must be a wide matrix (for the sake of efficiency)
# if `reg` is `:RMT` (default) the random matrix theory shrinkage is used.
# 	Any other ymbol will use the Ledoit & Wolf shrinkage.
# `tol` is the stopping criterion
...
function nrtmeFilt(X::AbstractMatrix{T}, sr;
                reg::Symbol = :RMT,
                tol::Real = real(T)(1e-6),
                maxiter::Int = 200,
                verbose::Bool = false) where {T<:Union{Real,Complex}}
    ...
    BPFilt = digitalfilter(Bandpass(1/(sr/2), 8/(sr/2)), Butterworth(2))
    Xf = filtfilt(BPFilt, X)

    x²f = [x⋅x for x ∈ eachcol(Xf)]
    Σxf = sum(x²f)

    x² = x²f*Σx/Σxf
    #X = Xf  #uniquement pour 2012Filter.csv, pas pour 2012GFPFilter.csv

# Calcul de la somme avant de tout faire, faire en sorte de conserver la somme (diviser par la nouvelle somme, multiplier par l'ancienne)
